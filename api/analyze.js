...
- "trade_idea" = แผนระยะยาว (Swing) อิงโครงสร้าง 4H เป็นหลัก (ใช้ 1H เป็นแค่ตัวช่วยยืนยัน) SL กว้างกว่าเพราะถือข้ามวัน/หลายวัน
- "trade_idea_short" = แผนระยะสั้น (Intraday) อิงโครงสร้าง 1H เป็นหลัก (ใช้ 15M เป็นแค่ตัวช่วยจับจังหวะเข้าให้แม่นขึ้น ไม่ใช่ตัวกำหนด SL) SL แคบกว่าแผนระยะยาว แต่กว้างกว่าถ้าอิง 15M ตรงๆ เพื่อลดโอกาสโดน noise สะบัดออกบ่อยเกินไป เหมาะเข้าไม้จบในวัน ถ้า bias ของ 1H เป็น Wait หรือโครงสร้างยังไม่ชัดเจนพอ ให้ใส่ '-' ทุกช่องได้

สำคัญมาก: ทุก field ที่เป็นข้อความต้องกระชับที่สุด — "structure_note" และ "smc_zones" ของแต่ละ timeframe ห้ามเกิน 1 ประโยคสั้นๆ (ไม่เกิน ~25 คำ), "overall_structure_note" และ "plan_summary" ห้ามเกิน 2 ประโยคสั้นๆ, "confidence_note" ห้ามเกิน 1 ประโยค, "note" ใน trade_idea_short ห้ามเกิน 1 ประโยคสั้นๆ (ไม่เกิน ~15 คำ) ห้ามใส่รายละเอียดราคาซ้ำกับ support/resistance levels ที่มีอยู่แล้ว เพราะคำตอบต้องจบเป็น JSON ที่สมบูรณ์เสมอ ห้ามถูกตัดกลางคัน
...
ตอบกลับเป็น JSON ล้วนๆ เท่านั้น (ไม่มีข้อความอื่นนอก JSON ไม่มี markdown code fence) ตามโครงสร้างนี้เป๊ะๆ:
{
  ...
  "confidence_score": 0,
  "trade_idea": {
    "entry_zone": "ช่วงราคาที่น่าสนใจถ้ามีโครงสร้างสนับสนุน อิงโครงสร้าง 4H เป็นหลัก เช่น '4478-4487' ถ้า bias เป็น Wait ให้ใส่ '-'",
    "stop_loss": "ราคา SL แบบกว้างๆ ตามโครงสร้าง 4H เผื่อ noise/liquidity sweep ปกติของทองคำ เหมาะกับถือข้ามวัน/หลายวัน หรือ '-'",
    "take_profit": ["TP1", "TP2", "TP3"],
    "rr_ratio": "เช่น '1:2.3' หรือ '-'"
  },
  "trade_idea_short": {
    "entry_zone": "ช่วงราคาสำหรับเข้าไม้ระยะสั้น อิงโครงสร้าง 1H เป็นหลัก (ดูโซน 15M ประกอบแค่ช่วยจับจังหวะเข้าให้แม่นขึ้น) เช่น '4489-4494' ถ้าโครงสร้าง 1H ยังไม่ชัดเจนพอให้ใส่ '-'",
    "stop_loss": "ราคา SL แคบกว่า trade_idea (4H) แต่กว้างกว่าถ้าอิงจุด swing ของ 15M ตรงๆ — วางเลยจุด swing สูง/ต่ำล่าสุดของ 1H ไปพอประมาณ เพื่อกันโดนสะบัดจาก noise ของ 15M เหมาะเทรดจบในวัน หรือ '-'",
    "take_profit": ["TP1", "TP2"],
